...
**Slice 1 (Foundation + Garage tab) is being executed** from a written plan. Status as of 2026-09-06 (session 4): every code task (1, 3–21 plus the overlay mount) is complete and verified (22 jest suites / 113 tests, typecheck clean); only the hardware-bound Tasks 2, 22 and 23 remain. **The emulator blocker is cleared** — an arm64-v8a AVD now exists, but it is named **`s24ultraProxy`**, not `s24proxy`; substitute that name wherever a brief, the plan, or the notes below say `s24proxy`. Verify with `grep abi.type "$USERPROFILE/.android/avd/s24ultraProxy.avd/config.ini"` (says `arm64-v8a`, Pixel 8, `google_apis_playstore_ps16k`). To resume: dispatch Task 2 Steps 3–6 from `task-2-brief.md`, then 22, then 23. Before doing anything else, read, in this order:

0. `WHERE-WE-LEFT-OFF.md` at the repo root — the plain-language handoff note, including the AVD rename.
...
Environment already done: `ANDROID_HOME` + PATH persisted as user env vars (new shells only — in this session call `"$LOCALAPPDATA/Android/Sdk/platform-tools/adb.exe"` and `.../emulator/emulator.exe` by absolute path); **This PC is ARM64 Windows (Snapdragon X)** — only `arm64-v8a` system images run; x86_64 AVDs exit immediately. The AVD is **`s24ultraProxy`** — Pixel 8 on the API 35 Google Play **arm64-v8a** image (`google_apis_playstore_ps16k`), confirmed present on 2026-09-06. The earlier `s24proxy` (x86_64) is gone. Git Bash reports `x86_64` because it runs emulated — don't be fooled; `systeminfo` says `ARM64-based PC`. No `cmdline-tools` in the SDK.
