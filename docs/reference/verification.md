...
## iOS — first run on an iPhone (Expo Go 57, through the cloudflared tunnel), 2026-09-18

Not a fidelity pass — the only thing judged so far is that the app launches. Two findings, both fixed or documented the same day:

1. **The Expo Go login gate.** Expo Go 57 on iOS refuses any dev server that is not signed in to the same Expo account as the phone's Expo Go (Expo changelog "Login now required for running projects in Expo Go"; iOS only so far). The recipe is in the Slice 3 tunnel note above and in `WHERE-WE-LEFT-OFF.md`. The earlier "sign out of Expo Go" advice was wrong and has been removed.
2. **The app opened on expo-router's black "Unmatched Route" page** (`Page could not be found`, with the app's own `exp://…/--/` URL and Go back · Sitemap). Root cause: `app/` had no root index route — `app/(tabs)/…` only — so a launch that reaches the router as the root path `/` matched the router's built-in not-found catch-all (`matchForEmptyPath` falls through to the `*not-found` leaf when no index leaf exists). Android never showed it because Expo Go there hands the router no path at all, and the tab navigator's default (garage) applies; Expo Go on iOS hands over an explicit root URL. Proof: a scratch jest probe (deleted, not committed) rendered a mock context mirroring `app/`'s tree through `expo-router/testing-library`'s `renderRouter` for sixteen launch-URL variants — root, trailing slashes, the Expo Go `/--/` forms, a query string, a bogus path — and every root-path variant resolved to segments `["+not-found"]` with the Unmatched screen mounted. **Fix:** `app/index.tsx` and `app/+not-found.tsx`, each `<Redirect href="/(tabs)/garage" />`. With them the same probe mounted the Unmatched screen for none of the variants. Gate after the fix: 49 suites / 381 tests, `tsc --noEmit` clean. Metro's log confirmed the iPhone pulling the iOS bundle (`iOS Bundled … expo-router\entry.js (2337 modules)`) and the fast-refresh updates that followed the new route files.

Still to judge on iOS, by eye: the session-8 iOS rendering paths (tilt-map pin shadow, the bubble highlight's SVG blur), the Skia splash, the Splash/Auth glass, the sheets, and the scanner (camera permission copy, the EXIF-orientation crop, the WebView OCR engine). Append findings here.
